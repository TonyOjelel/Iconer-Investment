...
const RechargesHistory = () => {
   ...
   const getHistory = async () => {
      const response = await axios.post('https://ikoner-node.glitch.me/recharge/history', {
                username: username
           });
   ...
   const Page = () => {
     document.title = 'Recharge History | Ikoner';
    };
   ...
   return (
      <div className="hero has-background-grey-light is-fullheight is-fullwidth">
         <div className="methods3">
          <h1 className="tasks-center">Recharge Record</h1>
         <img alt="sync" className="sync-red" src={Sync} onClick={getHistory} width={25}/>
          </div>
           { story && <div className="">
                {story.map((rghistory, index) => (
                     <div className="td-data-global" key={rghistory.id}>
                          <div className="td-data-global-row">
                            <div className="withdraw-mini">{rghistory.status}</div>
                            <div className="separater2"/>
                            <h1 className="datetime">{rghistory.updatedAt}</h1>
                          </div>
                          <div className="td-data-global-row">
                            <h1 className="amount-withdraw">Recharge Amount</h1>
                            <div className="separater2"/>
                            <h1 className="withdraw-amount">{rghistory.amount}.00 USDT</h1>
                          </div>
                     </div>
                 ))}
            </div> }
